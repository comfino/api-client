...
class ClientTest extends TestCase
{
    ...
    private string $productionApiHost;
    ...
    {
        $apiClient = $this->initApiClient('/v1/user/is-active', 'GET', responseBody: json_encode(true), apiKey: 'API-KEY');
        $response = $apiClient->isShopAccountActive();

        $this->assertTrue($response);

        $apiClient = $this->initApiClient('/v1/user/is-active', 'GET', responseBody: json_encode(false), apiKey: 'API-KEY');
        $response = $apiClient->isShopAccountActive();
    ...
    public function testGetFinancialProducts(): void
    {
        ...
        $this->assertEquals(
            new FinancialProduct(
                ...
                null,
                [
                    new Dto\Payment\LoanParameters(40000, 120000, 3, 0),
                    new Dto\Payment\LoanParameters(20000, 120000, 6, 0),
                    new Dto\Payment\LoanParameters(12000, 120000, 10, 0),
                ]
            ),
    ...
    public function testCreateOrder(): void
    {
        ...
        $order = new Order(
            ...
            new \Comfino\Shop\Order\Customer(
                ...
                null,
                null,
                new \Comfino\Shop\Order\Customer\Address('Rowowa', '13', '21', '10-899', 'Karłowice', 'PL')
            ),
            'https://comfino-shop.test/notification',
            new Seller('472156893')
        );

        $apiClient = $this->initApiClient('/v1/orders', 'POST', requestBody: $jsonRequest, responseBody: $jsonResponse, apiKey: 'API-KEY');
    ...
    public function testGetOrder(): void
    {
        ...
        $this->assertEquals(
            new Customer(
                ...
                null,
                null,
                new Address('Rowowa', '13', '21', '10-899', 'Karłowice', 'PL')
            ),
            $response->customer
        );
    ...
    public function testCancelOrder(): void
    {
        ...
        $apiClient = $this->initApiClient(sprintf('/v1/orders/%s/cancel', $orderId), 'PUT', apiKey: 'API-KEY');
    ...
    public function testGetProductTypes(): void
    {
        ...
        $apiClient = $this->initApiClient('/v1/product-types', 'GET', responseBody: json_encode($productTypesWithNames), apiKey: 'API-KEY');
    ...
    public function testGetWidgetKey(): void
    {
        ...
        $apiClient = $this->initApiClient('/v1/widget-key', 'GET', responseBody: json_encode($widgetKey), apiKey: 'API-KEY');
    ...
    public function testGetWidgetTypes(): void
    {
        ...
        $apiClient = $this->initApiClient('/v1/widget-types', 'GET', responseBody: json_encode($widgetTypesWithNames), apiKey: 'API-KEY');
    ...
    private function initApiClient(string $endpointPath, string $method, ?array $queryParameters = null, ?string $requestBody = null, ?string $responseBody = null, ?string $apiKey = null, bool $isPublicEndpoint = false, int $responseStatus = 200): Client
    {
        $client = new \Http\Mock\Client();
        $client->on(
            new RequestMatcher($endpointPath, $this->productionApiHost, $method, 'https'),
            fn (RequestInterface $request) => $this->processRequest($request, $queryParameters, $requestBody, $responseBody, $apiKey, $isPublicEndpoint, $responseStatus)
        );

        $httpFactory = new HttpFactory();

        return new Client($httpFactory, $httpFactory, $client, $apiKey);
    }

    private function processRequest(RequestInterface $request, ?array $queryParameters = null, ?string $requestBody = null, ?string $responseBody = null, ?string $apiKey = null, bool $isPublicEndpoint = false, int $responseStatus = 200): ResponseInterface
    {
        if (!$isPublicEndpoint && (!$request->hasHeader('Api-Key') || $request->getHeaderLine('Api-Key') !== $apiKey)) {
            return new \GuzzleHttp\Psr7\Response(401, ['Content-Type' => 'application/json'], '{"message":"Invalid credentials."}');
        }
        ...
        return new \GuzzleHttp\Psr7\Response($responseStatus, ['Content-Type' => 'application/json'], $responseBody);
    }
}
